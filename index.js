...
function initMap() {
  ...
  map.on('overlayadd overlayremove', function(e) {
    ...
    if (uasFacilityMapLayer) {
      ...
      if (uasFacilityMapEnabled) applyLaancZoomGate();
    }
    updateAirspaceLegend(e);
  });

  // Zoom-gate: show LAANC only at zoom >= LAANC_MIN_ZOOM
  map.on('zoomend', function() {
    if (uasFacilityMapEnabled && uasFacilityMapLayer) applyLaancZoomGate();
  });
...
// Apply or remove LAANC data based on current zoom level.
...
function applyLaancZoomGate() {
  if (!uasFacilityMapLayer) return;
  const zoom = map.getZoom();
  if (zoom >= LAANC_MIN_ZOOM) {
    // Enable data fetching (empty where = all features in viewport)
    uasFacilityMapLayer.setWhere('');
  } else {
    // Block all data fetching and clear existing features
    uasFacilityMapLayer.setWhere('1=0');
  }
