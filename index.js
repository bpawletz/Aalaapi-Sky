...
const FlightDiagnostics = {
  ...
  addSatelliteFloor() {
    if (!this.telemetryData || !this.telemetryData.points || !this.telemetryData.points.length || typeof document === 'undefined' || !document.createElement) return;
    const origin = this.telemetryData.points[0];
  ...
  buildTrajectoryMeshes() {
    ...
    const projectToWorld = (lat, lon, alt) => {
    ...
    const actualCoords = [];
    pts.forEach(p => {
      actualCoords.push(projectToWorld(p.lat, p.lon, p.alt));
    });
    ...
    wps.forEach(wp => {
      plannedCoords.push(projectToWorld(wp.lat, wp.lon, wp.altitude || 21.0));
    });
  ...
  animate() {
    ...
    if (this.isPlaying && this.telemetryData && this.telemetryData.points.length > 0) {
      ...
      if (this.lastFrameTime) {
        ...
        this.seekTo(Math.floor(this.currentPointIndex), true);
      }
  ...
  play() {
    if (this.telemetryData && this.telemetryData.points) {
      if (this.currentPointIndex >= this.telemetryData.points.length - 1) {
  ...
  seekTo(index, updateSlider = true) {
    ...
    const sinLat = Math.sin(pt.lat * Math.PI / 180);
    const xTile = ((pt.lon + 180) / 360) * Math.pow(2, tileZoom);
    const yTile = (0.5 - Math.log((1 + sinLat) / (1 - sinLat)) / (4 * Math.PI)) * Math.pow(2, tileZoom);
